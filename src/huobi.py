...
def get_fund_list(cookie_str=None):
    ...
    output_head = '代码' + ',' + '晨星专属号' + ',' + '名称' + ',' + '类型' + ',' + '三年评级' + ',' + '五年评级' + ',' + '今年回报率' + ',' + '基金规模' + ',' + '基金公司' + ',' + '基金公司规模' + ',' + '基金经理' + ',' + '任职时间' + ',' + '天天基金经理评分' + ',' + '好买基金经理评分' + '\n'
    # 设置表头
    if page_num == 1:
        with open(result_dir + 'fund_morning_star_huobi.csv', 'w+') as csv_file:
...
def writeCsvAndGetFundDetail(chrome_driver, allData):
    print('开始组织表格数据')
    with open(result_dir + 'fund_morning_star_huobi.csv', 'a') as csv_file:
        for fund_item in allData:
            ...
            output_line += ',' + str(totalFree) + ',' + str(company) + ',' + str(companyFree) + ',' + str(fundManager) + ',' + str(manager_time) + ',' + str(tt_fund_manager_roate) + ',' + str(howbuy_manager_roate) + '\n'
            sleep(2)
